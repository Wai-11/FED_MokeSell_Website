...
const gameVoucherPromoCodes = [
    {name : 'WINNER10', discount : 10},
    {name : 'MOKESELL', discount : 15},
    {name : 'CNY25', discount : 25},
    {name : 'NEWYEAR25', discount : 27},
    {name : 'MOKE', discount : 5},
...
*/
const APIKEY = "67a8b1a799fb60ff4ce983e9";
...
if (checkout) {
    checkout.addEventListener("click", function (e) {
        ...
        if (getAccount() === false) {
            ...
            // bulk insert
            fetch("https://mokesell-7706.restdb.io/rest/orders", settings)
...
function updateMemberInfo(listOfItems) {
    ...
    fetch(`https://mokesell-7706.restdb.io/rest/membership?q={"name":"${name}"}`, {
        method: "GET",
        headers: {
    ...
    .then(data => {
        ...
        if (membership) {
            ...
            return fetch(`https://mokesell-7706.restdb.io/rest/membership/${membership._id}`, updateSettings);
        } else {
...
async function getMembershipByName(name) {
    // Fetch the current membership information
    return await fetch(`https://mokesell-7706.restdb.io/rest/membership?q={"name":"${name}"}`, {
        method: "GET",
        headers: {
            "Content-Type": "application/json; charset=utf-8",
            "x-apikey": APIKEY,
...
var removeCode = document.getElementById("removeCode");
if (removeCode) {
